...
3. Bump both `v0.x.y` mentions in `README.md`'s **Install** section (the
   "Current release" line and the APK filename). The release badge is
   generated by shields.io and needs no edit.
...
- **`ProxyService` is a `specialUse` foreground service, not `dataSync`.**
  ...
  review, but DataProxy ships via GitHub Releases, so runtime is unaffected -
...
- **Anti-Kill is a header chip, not a Home body element.** A small
  `🛡 Anti-Kill` accent chip sits in the top-right of the Home header,
  stacked *under* the carrier/tech indicator (in `Header()`). The user tried
  a body card/row first and rejected both ("ugly af"), keep it in the
  header; don't add a fourth nav tile or a body card for it.
- **Listen / Devices / Auth / Anti-Kill are separate screens** reached via
  the nav tiles (Listen/Devices/Auth) or the header chip (Anti-Kill), not
  embedded sections. Each one calls `BackHandler(onBack = onBack)` at the top
  so hardware back returns to Home instead of finishing the activity.
...
- **Theme toggle is a 36-dp icon button under the bolt logo** in the home
  header. Cycles `System to Light to Dark to System`. Persisted as
...
DataProxy has a single foreground service (the proxy itself), there is **no**
...
- **`AntiKillPreferences`** (`util/`) stores `autostart_on_boot` and the
  manual `antikill_step_<Name>` flags in `dataproxy_prefs`. Auto-detectable
